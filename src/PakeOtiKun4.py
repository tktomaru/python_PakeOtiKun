...
def tc_set (dev, delay, loss, dup, corrupt):
...
def lcd_write (delay, loss, dup, corrupt, select):
    if select == 0:
...
if __name__=="__main__":
    ...
    while (1):
        ...
        if (d != delay or l != loss or p != dup or c != corrupt):
            delay = d
            loss = l
            dup = p
            corrupt = c
            for n in range(devnum):
                tc_set(devices[n], delay, loss, dup, corrupt)
            lcd_clear()
            lcd_write (delay, loss, dup, corrupt, gselect)
